...
const codeSnippet3 = `exports.commentVote = async (req, res) => {
    let { commentid, type } = req.params
    ...
    try {
        let comment = await RetroComment.findById(commentid).lean().exec()

        if (type === 'upVote') {
            if (comment.upVotes.map(x => x.toString()).includes(req.user._id)) {
                let updated = await RetroComment.findByIdAndUpdate(commentid, { $pull: { upVotes: req.user._id } }, { new: true })
                    .select('upVotes')
                    .exec()
                res.status(201).json({ success: true, comment: updated })
            } else {
                let updated = await RetroComment.findByIdAndUpdate(commentid, { $push: { upVotes: req.user._id } }, { new: true })
                    .select('upVotes')
                    .exec()
                res.status(201).json({ success: true, comment: updated })
            }
        } else if (type === 'downVote') {
            if (comment.downVotes.map(x => x.toString()).includes(req.user._id)) {
                let updated = await RetroComment.findByIdAndUpdate(commentid, { $pull: { downVotes: req.user._id } }, { new: true })
                    .select('downVotes')
                    .exec()
                res.status(201).json({ success: true, comment: updated })
            } else {
                let updated = await RetroComment.findByIdAndUpdate(commentid, { $push: { downVotes: req.user._id } }, { new: true })
                    .select('downVotes')
                    .exec()
                res.status(201).json({ success: true, comment: updated })
            }
...
const list3 = [
    "The function is exported as commentVote and receives req (request) and res (response) objects as parameters, which are typically provided by the Express.js framework.",

    "The function extracts commentid and type from the params object in the request. These values are expected to be provided as URL parameters.",

    "The function performs some basic validation checks: If commentid is falsy (empty or undefined), it sends a 400 Bad Request response with an error message stating that the data is invalid, If type is falsy, it sends a 400 Bad Request response with an error message stating that the data is invalid.",

    "If the input data passes the validation checks, the function proceeds to handle the comment vote based on the type. The function performs the following actions: It retrieves the comment with the specified commentid using RetroComment.findById(). The lean() method is used to get a plain JavaScript object instead of a Mongoose document, and exec() is used to execute the query, If the type is 'upVote', it checks if the user's ID is already present in the comment's upVotes array. If so, it removes the user's ID from the upVotes array using $pull and returns the updated comment data, If the user's ID is not present in the upVotes array, it adds the user's ID to the upVotes array using $push and returns the updated comment data, If the type is 'downVote', it follows a similar logic to handle the downVotes array.",

    "If any error occurs during the process (e.g., an error while querying the database), it catches the error, logs it to the console, and sends a 400 Bad Request response with a generic error message stating that something went wrong.",
]
...
const AgileExecution = () => {
  const userStory = {
      title: 'Retro board like and dislike functionality adding.',
      assignedTo: 'Rafiur Rahman Protik',
      description: {
          title: "As a developer, I want to implement the make retro board like and dislike functionality in AgileALM so that users can engage in anyone's task.",
          list: [
              "Show Like and Dislike button.",
               "Like and dislike icons must be clickable",
               "Make an API to work it.",
              ]
      },
      priority: "1",
      risk: "1",
      ...
      <Box>
          ...
          <Box mt={2}>
              <Box sx={{ display: 'flex', justifyContent: 'center' }}>
                  <ImageComp image="/agile/retro_like.png"></ImageComp>
              </Box>
              ...
              <Typography variant='h4' mt={2}>Here we give some information related to retro borad like and dislike user story:
              </Typography>
...
const ImageInterface = () => {
    return (
        <ImageComp image={"/agileALM/retroLike.png"} imageTitle={"Screenshot 1: Board View"}></ImageComp>
    )
}
...
const BackendComp = () => {
    return (
        <Box>
            <Typography mt={2} variant='h4'>commentVote controller function</Typography>
            {/* <StepComp stepText="Login.js"></StepComp> */}
